...
export class WorksService {
    ...
    updateWorksItemParticipants(
        ...
        return from(this.worksModel.findByIdAndUpdate(id, worksItemParticipantsDto))
            .pipe(
                switchMap(() => {
                    return from(this.participantModel.findByIdAndUpdate(participantId, { $push: { works: id } }));
                })
            );
    }

    deleteWorksItem(id: string): Observable<UpdateWriteOpResult> {
        return from(this.worksModel.findByIdAndRemove(id))
            .pipe(
                switchMap(() => {
                    return this.participantModel.updateMany({}, {$pull: {works: id}});
                })
            );
    }
